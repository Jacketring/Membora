<div class="page-heading">
  <div>
    <h2>Membresias</h2>
    <p>Gestiona planes, precios, duracion y vencimientos de los socios.</p>
  </div>
  <button class="primary-action primary-action--compact" data-open-modal="membership-modal" type="button">Nueva membresia</button>
</div>

<section class="lead-metrics" aria-label="Resumen de membresias">
...
<form class="lead-toolbar membership-toolbar" method="get" aria-label="Filtros de membresias" data-auto-filter-form data-live-search-form data-live-search-target="memberships-table">
  <input type="hidden" name="route" value="memberships">
  <label class="lead-search">
    <span>Buscar</span>
    <input name="q" value="<?= e($filters['q']) ?>" placeholder="Nombre de membresia o socio" aria-label="Buscar membresias o socios" data-auto-filter-input>
  </label>
...
<section class="leads-table-card">
  <header>
    <div>
      <h3>Planes de membresia</h3>
      <span data-live-search-count><?= count($plans) ?> resultados</span>
    </div>
  </header>
  <div class="leads-table-wrap">
    <table class="leads-table" id="memberships-table">
      <caption class="sr-only">Listado de planes de membresia con precio y duracion</caption>
      <thead>
        <tr>
      ...
      <tbody>
        <?php foreach ($plans as $plan): ?>
          <tr class="lead-data-row clickable-row" data-open-modal="membership-detail-<?= e($plan['id']) ?>" data-live-search-row>
            <td>
              ...
              <small class="table-subtext"><?= e($plan['description'] ?: 'Sin descripcion') ?></small>
            </td>
            ...
            <td>
              <div class="row-actions">
                <button class="icon-action" data-open-modal="membership-detail-<?= e($plan['id']) ?>" type="button" title="Editar membresia" aria-label="Editar membresia <?= e($plan['name']) ?>">
                ...
                <form method="post" data-confirm-message="Eliminar esta membresia? Solo se permite si no esta asignada a socios.">
                  <input type="hidden" name="action" value="delete_membership_plan">
                  <input type="hidden" name="id" value="<?= e($plan['id']) ?>">
                  <button class="icon-action danger-action" type="submit" title="Eliminar membresia" aria-label="Eliminar membresia <?= e($plan['name']) ?>">
        ...
        <?php if (!$plans): ?>
          <tr data-live-search-empty>
            <td class="leads-empty-cell" colspan="5">No hay membresias que coincidan con los filtros actuales.</td>
          </tr>
        <?php endif; ?>
      </tbody>
    </table>
...
<section class="leads-table-card membership-subscriptions-card">
  <header>
    <div>
      <h3>Socios con membresia</h3>
      <span><?= count($subscriptions) ?> asignaciones activas</span>
    </div>
  </header>
  <div class="leads-table-wrap">
    <table class="leads-table">
      <caption class="sr-only">Socios con membresia asignada y fecha de caducidad</caption>
      <thead>
        <tr>
...
<?php foreach ($plans as $plan): ?>
  <dialog id="membership-detail-<?= e($plan['id']) ?>" class="modal-card" aria-labelledby="membership-title-<?= e($plan['id']) ?>">
    <form method="post" aria-label="Editar membresia <?= e($plan['name']) ?>">
...
<dialog id="membership-modal" class="modal-card" aria-labelledby="membership-modal-title">
  <form method="post">
    ...
    <header>
      <h2 id="membership-modal-title">Nueva membresia</h2>
      <button data-close-modal type="button">Cerrar</button>
    </header>
    <?php require __DIR__ . '/partials/membership-plan-form.php'; ?>
    <button class="primary-action" type="submit">Crear membresia</button>
  </form>
</dialog>
